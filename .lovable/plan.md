...
## Turn 4 debt rows (harness + fixtures shipped; UX + Playwright body deferred)

- **#24** — Turn 4 shipped the DB-mock harness (`src/test/db-mock.ts` + 4 self-tests) and 13 route-handler fixture assertions across 6 fixture files: `held-slot-race`, `drop-validate-rules` (4), `overbook-two-paths` (3), `book-atomic` (2), `no-eligibility-without-coverage` (2), `eligibility-post-book` (1). Full suite 91/0.
- **#25** — Handler pure-fn refactor completed for 4 routes: `scheduler.validate-drop`, `scheduler.bookings.$id.book`, `scheduler.bookings.$id.status`, `scheduler.bookings.$id.eligibility-check`. Remaining routes (`scheduler.board`, `scheduler.booking-requests`, `scheduler.blocks`) still keep logic inline in the Route wrapper — no fixture presses on them yet, so Turn 5 refactors these two: `booking-requests` for the union fixture, `board` on demand.
- **#26** — Two of the 8 planned fixtures are deferred to Turn 5: `charge-mode-display` (needs `pricing_rule` condition/action fixture format matching `evaluateTriggers` — reproduce inline via a small helper) and `booking-requests-union` (needs `booking-requests` refactor per #25). Neither blocks Turn 4 shipping since both are behavior we already exercise elsewhere in integration.
- **#27** — DayBoard `data-*` attributes shipped (`data-tone`, `data-column-index`, `data-testid="session-title"`, `data-testid="now-line"`, `data-overbook-ribbon`). ClinicDayBoardPane got `data-testid="walk-in-lane"` and `data-testid="booking-request-card"`. Playwright body enhancements (7 assertion blocks + snapshot with now-line mask) and `seedSchedulerFixtures()` in `demo-seed.functions.ts` are deferred to Turn 5 — the existing `day-board.spec.ts` skeleton continues to skip cleanly under `DEMO_USER_PASSWORD`.
- **#28** — Drag-drop UX in `ClinicDayBoardPane` (rail → slot with `dataTransfer`, `errorLabel` toast map, overbook-ribbon setter, walk-in specialty popover, keyboard-Enter equivalence) deferred to Turn 5. Current pane is read-only; validate-drop is fully callable via `schedulerApi.validateDrop` when the UI lands.