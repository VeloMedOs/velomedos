...
export const SERVICES = [
  ...
  {
    slug: "homecare",
    title: "Homecare Service — verified in-home nursing visits",
    short: "Recurring in-home nursing and caregiver visits — scheduled, routed, and geofenced check-in/out, with vitals, medications and care-plan tasks captured at the bedside.",
    keyword: "homecare visit verification platform",
    benefits: [
      "EVV (Electronic Visit Verification) — geofenced, tamper-proof proof every visit happened.",
      "Skills-matched assignment so only credentialed caregivers reach the bedside.",
      "Vitals, medication administration (MAR) and care-plan tasks captured live in the field app.",
      "Family visibility — live ETA, who's coming, visit history and vitals trend.",
      "Missed-visit alerts with one-tap reassign to a skills-matched caregiver.",
      "Payer- and regulator-ready audit export (CSV/PDF) per branch.",
    ],
    how: [
      "Build a care plan with frequency, required skills and a task template.",
      "Caregiver routed to the home; live location shared with dispatch and family.",
      "Geofenced Check-In; out-of-range attempts capture an EVV exception reason.",
      "In-visit cockpit: care-plan tasks, vitals, medications and notes at the bedside.",
      "Check-Out runs EVV (distance + time window) and finalises the verified visit.",
    ],
    faqs: [
      { q: "What does EVV actually verify?", a: "Each visit is verified by the caregiver's GPS at check-in/out (within a configurable geofence radius of the recipient's home) and by time window against the scheduled start/end. Only verified visits emit billable usage." },
      { q: "Can caregivers check in when GPS is bad?", a: "Yes. Out-of-range check-ins are allowed but flagged with a recorded exception reason; the EVV audit export distinguishes them so reviewers can decide policy." },
      { q: "Is the public developer API gone?", a: "No — it is still live and documented at /superadmin/api-docs (operator) and the documented infrastructure that powers every VeloMed surface. Homecare adds new endpoints under the `Homecare` tag with `homecare:read` and `homecare:write` scopes." },
      { q: "Which plans include Homecare?", a: "Homecare is included in Professional and Enterprise (off by default on Essential) and metered per EVV-verified visit, with optional add-ons for the EVV Compliance Pack, Family Portal, MAR and route optimisation." },
    ],
  },
] as const;
