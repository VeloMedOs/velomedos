...
export const openApiClinicalSpec = {
  ...
  paths: {
    ...
    "/claims/{id}/ready": {
      parameters: [{ name: "id", in: "path", required: true, schema: { type: "string", format: "uuid" } }],
      post: { tags: ["Claims"], summary: "Transition draft → ready (biller QA gate). Phase 10 enforces strict MDS + RCM completeness; on failure returns 422 with the precise missing items.", responses: { 200: { description: "Ready", content: { "application/json": { schema: { type: "object" } } } }, 409: { description: "bad_status", content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } }, 422: { description: "mds_incomplete | rcm_incomplete — payload includes missing[] grouped by stage", content: { "application/json": { schema: { type: "object", properties: { error: { type: "string" }, code: { type: "string" }, request_id: { type: "string" }, missing: { type: "array", items: { type: "object", properties: { code: { type: "string" }, category: { type: "string" }, stage: { type: "string", enum: ["mds", "drg", "rcm"] }, message: { type: "string" }, severity: { type: "string", enum: ["error", "warning"] } } } } } } } } } } },
    },
    "/claims/{id}/submit": {
      parameters: [{ name: "id", in: "path", required: true, schema: { type: "string", format: "uuid" } }],
      post: { tags: ["Claims"], summary: "Phase-9 NPHIES submit. Single-flight via Idempotency-Key header. Phase 10 re-validates MDS + RCM defensively; on failure returns 422 with missing[].", parameters: [{ name: "Idempotency-Key", in: "header", required: false, schema: { type: "string" } }], requestBody: { required: false, content: { "application/json": { schema: { type: "object", properties: { note: { type: "string" } } } } } }, responses: { 200: { description: "Adjudication outcome + reconciliation deltas + attempt id", content: { "application/json": { schema: { type: "object" } } } }, 409: { description: "bad_status | submission_in_flight", content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } }, 422: { description: "mds_incomplete | rcm_incomplete", content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } }, 502: { description: "gateway_error", content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } } } },
    },
    "/claims/{id}/completeness": {
      parameters: [{ name: "id", in: "path", required: true, schema: { type: "string", format: "uuid" } }],
      get: { tags: ["Claims"], summary: "Phase 10 — read-only readiness probe. Returns MDS, DRG and RCM completeness flags + the exact missing items the provider UI shows.", responses: { 200: { description: "Completeness report", content: { "application/json": { schema: { type: "object", properties: { ok: { type: "boolean" }, missing: { type: "array", items: { type: "object" } }, drg: { type: "object" }, rcm: { type: "object" } } } } } }, 404: { description: "not_found", content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } } } },
    },
    "/claims/{id}/attempts": {
      parameters: [{ name: "id", in: "path", required: true, schema: { type: "string", format: "uuid" } }],
