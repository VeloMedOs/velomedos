...
export async function handleGET(args: {
  ...
  if (query.clinic_id) scheduleQ = scheduleQ.eq("clinic_id", query.clinic_id);
  const { data: schedules, error: sErr } = await scheduleQ;
  if (sErr) return envelope("database_error", "db_error", 500);

  const providerIds = [...new Set((schedules ?? []).map((s: any) => s.provider_id).filter(Boolean))] as string[];
  const { data: providers } = providerIds.length
    ? await db.from("providers").select("id, display_name").in("id", providerIds)
    : { data: [] };
  const providerName = new Map((providers ?? []).map((p: any) => [p.id, p.display_name]));

  const columns = (schedules ?? []).map((s: any) => ({
    session_id: s.id,
...
export const Route = createFileRoute("/api/clinical/v1/scheduler/board")({
  server: { handlers: {
    ...
    GET: async ({ request }) => {
      ...
      return handleGET({ query: parsed.data, ctx: auth.ctx });
    },
  } },
});