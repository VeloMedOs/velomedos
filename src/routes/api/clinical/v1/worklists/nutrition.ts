...
/**
 * GET /api/clinical/v1/worklists/nutrition
 * Encounters with open specialty (nutrition) forms — joined via
 * v_clinical_forms_worklist ∩ v_doctor_worklist.
 */
export const Route = createFileRoute("/api/clinical/v1/worklists/nutrition")({
  server: { handlers: {
    OPTIONS: () => preflight(),
    GET: async ({ request }) => {
      const auth = await requireClinicalModule(request, "Clinical", { capId: "wl.nutrition.read" });
