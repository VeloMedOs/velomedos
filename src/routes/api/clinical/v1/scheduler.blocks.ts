...
export type BlockBody = z.infer<typeof Body>;
  ...
  if (body.notify_stakeholders && slotIds.length) {
    ...
    if (bkngs?.length) {
      await db.from("booking_event").insert(
        bkngs.map((b: any) => ({
          tenant_id: ctx.tenantId, booking_id: b.id, event: "block_notified",
