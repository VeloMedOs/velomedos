  ...
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        let q = serviceClient().from("portal_config_overrides").select("*").order("updated_at", { ascending: false });
      ...
      PUT: async ({ request }) => {
        ...
        const { data, error } = await serviceClient().from("portal_config_overrides").upsert({
      ...
      DELETE: async ({ request }) => {
        ...
        const { error } = await serviceClient().from("portal_config_overrides").delete().eq("subscriber_id", sid).eq("key", key);
