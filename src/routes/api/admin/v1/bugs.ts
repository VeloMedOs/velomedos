  ...
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        const { data, error } = await serviceClient().from("portal_bugs").select("*").order("last_seen_at", { ascending: false }).limit(300);
      ...
      PATCH: async ({ request }) => {
        ...
        const { data, error } = await serviceClient().from("portal_bugs").update(upd).eq("id", body.id).select().single();
