  ...
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        const { data, error } = await adminDb().from("portal_promotions").select("*").order("created_at", { ascending: false });
      ...
      POST: async ({ request }) => {
        ...
        const { data, error } = await adminDb().from("portal_promotions").insert(body as never).select().single();
