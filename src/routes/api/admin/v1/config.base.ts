  ...
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        const { data, error } = await serviceClient().from("portal_config_base").select("*").order("key");
      ...
      PUT: async ({ request }) => {
        ...
        const { data, error } = await serviceClient().from("portal_config_base").upsert({ key: body.key, value: body.value as never, description: body.description ?? null, updated_by: auth.userId, updated_at: new Date().toISOString() }).select().single();
