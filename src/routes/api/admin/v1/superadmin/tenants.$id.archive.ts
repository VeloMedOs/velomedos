...
export const Route = createFileRoute("/api/admin/v1/superadmin/tenants/$id/archive")({
  server: {
    handlers: {
      OPTIONS: () => preflight(),
      POST: async ({ request, params }) => {
        ...
        if (row.tenant_lifecycle === "archived") return json({ error: "already_archived", code: "state/invalid_transition", request_id: crypto.randomUUID() }, 400);
        const { data, error } = await db.from("corporate_accounts").update({ tenant_lifecycle: "archived" }).eq("id", params.id).select().single();
        if (error) return json({ error: "database_error", code: "db/update_failed", request_id: crypto.randomUUID() }, 400);
        await adminAudit(auth.userId, "tenant.archive", "corporate_accounts", params.id, null);
        return json(data);
      },
    },
  },
